...
- Source file: `n8n-nodes-base/dist/nodes/Egoi/Egoi.node.js`
- Node version: `1`
- Groups: `output`
...
### Operations
- default/all resources: `create`, `get`, `getAll`, `update`
...
| List Name or ID | `list` | `options` | no |  | ID of list to operate on. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. |
| Email | `email` | `string` | no |  | Email address for a subscriber |
| Contact ID | `contactId` | `string` | no |  | Contact ID of the subscriber |
| Resolve Data | `resolveData` | `boolean` | no | `true` | By default the response just includes the contact ID. If this option gets activated, it will resolve the data automatically. |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Update Fields | `updateFields` | `collection` | no | `{}` |  |
| By | `by` | `options` | no | `id` | Search by |
| Contact ID | `contactId` | `string` | no |  | Contact ID of the subscriber |
| Email | `email` | `string` | no |  | Email address for subscriber |

...
- Source file: `n8n-nodes-base/dist/nodes/Pushover/Pushover.node.js`
- Node version: `1`
- Groups: `input`
...
### Operations
- resource `message`: `push`

## Parameters
| Display Name | Name | Type | Required | Default | Description |
|---|---|---|---|---|---|
| Resource | `resource` | `options` | no | `message` |  |
| Operation | `operation` | `options` | no | `push` |  |
| User Key | `userKey` | `string` | yes |  | The user/group key (not e-mail address) of your user (or you), viewable when logged into the <a href="https://pushover.net/">dashboard</a> (often referred to as <code>USER_KEY</code> in the <a href="https://support.pushover.net/i44-example-code-and-pushover-libraries">libraries</a> and code examples) |
| Message | `message` | `string` | yes |  | Your message |
| Priority | `priority` | `options` | no | `-2` | Send as -2 to generate no notification/alert, -1 to always send as a quiet notification, 1 to display as high-priority and bypass the user's quiet hours, or 2 to also require confirmation from the user |
| Retry (Seconds) | `retry` | `number` | yes | `30` | Specifies how often (in seconds) the Pushover servers will send the same notification to the user. This parameter must have a value of at least 30 seconds between retries. |
| Expire (Seconds) | `expire` | `number` | yes | `30` | Specifies how many seconds your notification will continue to be retried for (every retry seconds) |

...
- Source file: `n8n-nodes-base/dist/nodes/MySql/MySql.node.js`
...
| 
		In this mode, make sure incoming data fields are named the same as the columns in your table. If needed, use an 'Edit Fields' node before this node to change the field names.
		 | `notice` | `notice` | no |  |  |
| Column to Match On | `columnToMatchOn` | `options` | yes |  | Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/" target="_blank">expression</a> |
...
| Value of Column to Match On | `valueToMatchOn` | `string` | no |  | Rows with a value in the specified "Column to Match On" that corresponds to the value in this field will be updated. New rows will be created for non-matching items. |
| Values to Send | `valuesToSend` | `fixedCollection` | no | `{}` |  |
| Options | `options` | `collection` | no | `{}` |  |

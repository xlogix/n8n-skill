...
- Source file: `n8n-nodes-base/dist/nodes/PagerDuty/PagerDuty.node.js`
- Node version: `1`
- Groups: `output`
...
## Credentials
- `pagerDutyApi` (required)
- `pagerDutyOAuth2Api` (required)
...
### Operations
- resource `incident`: `create`, `get`, `getAll`, `update`
- resource `incidentNote`: `create`, `getAll`
- resource `logEntry`: `get`, `getAll`
- resource `user`: `get`
...
| Resource | `resource` | `options` | no | `incident` |  |
| Operation | `operation` | `options` | no | `create` |  |
| Title | `title` | `string` | yes |  | A succinct description of the nature, symptoms, cause, or effect of the incident |
| Service Name or ID | `serviceId` | `options` | yes |  | The incident will be created on this service. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. |
| Email | `email` | `string` | yes |  | The email address of a valid user associated with the account making the request |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Conference Bridge | `conferenceBridgeUi` | `fixedCollection` | no | `{}` |  |
| Incident ID | `incidentId` | `string` | yes |  | Unique identifier for the incident |
...
| Operation | `operation` | `options` | no | `create` |  |
| Incident ID | `incidentId` | `string` | yes |  | Unique identifier for the incident |
| Content | `content` | `string` | yes |  | The note content |
| Email | `email` | `string` | yes |  | The email address of a valid user associated with the account making the request |
| Incident ID | `incidentId` | `string` | yes |  | Unique identifier for the incident |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `100` | Max number of results to return |
| Operation | `operation` | `options` | no | `get` |  |
| Log Entry ID | `logEntryId` | `string` | yes |  | Unique identifier for the log entry |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `100` | Max number of results to return |
| Options | `options` | `collection` | no | `{}` |  |
| Operation | `operation` | `options` | no | `get` |  |
| User ID | `userId` | `string` | yes |  | Unique identifier for the user |

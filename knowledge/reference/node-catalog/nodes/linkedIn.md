...
- Source file: `n8n-nodes-base/dist/nodes/LinkedIn/LinkedIn.node.js`
- Node version: `1`
- Groups: `input`
...
## Credentials
- `linkedInOAuth2Api` (required)
- `linkedInCommunityManagementOAuth2Api` (required)
...
|---|---|---|---|---|---|
| Authentication | `authentication` | `options` | no | `standard` |  |
| Resource | `resource` | `options` | no | `post` |  |
| Operation | `operation` | `options` | no | `create` |  |
| Post As | `postAs` | `options` | no | `person` | If to post on behalf of a user or an organization |
| Person Name or ID | `person` | `options` | yes |  | Person as which the post should be posted as. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. |
| Organization URN | `organization` | `string` | no |  | URN of Organization as which the post should be posted as |
| Text | `text` | `string` | no |  | The primary content of the post |
| Media Category | `shareMediaCategory` | `options` | no | `NONE` |  |

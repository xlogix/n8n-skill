...
- Source file: `n8n-nodes-base/dist/nodes/Okta/Okta.node.js`
- Node version: `1`
- Groups: `transform`
...
| User | `userId` | `resourceLocator` | yes | `{"mode":"list","value":""}` | The user you want to operate on. Choose from the list, or specify an ID. |
| First Name | `firstName` | `string` | yes |  |  |
| Last Name | `lastName` | `string` | yes |  |  |
| Username | `login` | `string` | yes |  |  |
| Email | `email` | `string` | yes |  |  |
| Activate | `activate` | `boolean` | no | `true` | Whether to activate the user and allow access to all assigned applications |
| Fields | `getCreateFields` | `collection` | no | `{}` |  |
| Fields | `getUpdateFields` | `collection` | no | `{}` |  |
| Search Query | `searchQuery` | `string` | no |  |  |
...
| Send Email | `sendEmail` | `boolean` | no | `false` | Whether to send a deactivation email to the administrator |

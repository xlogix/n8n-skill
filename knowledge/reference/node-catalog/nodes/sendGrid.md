...
- Source file: `n8n-nodes-base/dist/nodes/SendGrid/SendGrid.node.js`
- Node version: `1`
- Groups: `transform`
...
|---|---|---|---|---|---|
| Resource | `resource` | `options` | yes | `list` |  |
...
| Name | `name` | `string` | yes |  | Name of the list |
| List ID | `listId` | `string` | yes |  | ID of the list |
| Delete Contacts | `deleteContacts` | `boolean` | no | `false` | Whether to delete all contacts on the list |
| List ID | `listId` | `string` | yes |  | ID of the list |
| Contact Sample | `contactSample` | `boolean` | no | `false` | Whether to return the contact sample |
| List ID | `listId` | `string` | yes |  | ID of the list |
| Name | `name` | `string` | yes |  | Name of the list |
...
| Email | `email` | `string` | yes |  | Primary email for the contact |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Contact IDs | `ids` | `string` | no |  | ID of the contact. Multiple can be added separated by comma. |
| Delete All | `deleteAll` | `boolean` | no | `false` | Whether all contacts will be deleted |
| By | `by` | `options` | yes | `id` | Search the user by ID or email |
| Contact ID | `contactId` | `string` | yes |  | ID of the contact |
| Email | `email` | `string` | yes |  | Email of the contact |
| Operation | `operation` | `options` | no | `send` |  |
| Sender Email | `fromEmail` | `string` | no |  | Email address of the sender of the email |
| Sender Name | `fromName` | `string` | no |  | Name of the sender of the email |
| Recipient Email | `toEmail` | `string` | no |  | Comma-separated list of recipient email addresses |
| Subject | `subject` | `string` | no |  | Subject of the email to send |
| Dynamic Template | `dynamicTemplate` | `boolean` | yes | `false` | Whether this email will contain a dynamic template |
| MIME Type | `contentType` | `options` | no | `text/plain` | MIME type of the email to send |
| Message Body | `contentValue` | `string` | yes |  | Message body of the email to send |
| Template Name or ID | `templateId` | `options` | no | `[]` | Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a> |
| Dynamic Template Fields | `dynamicTemplateFields` | `fixedCollection` | no | `{}` |  |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |

...
- Node name: `perplexity`
- n8n-nodes-base version: `2.7.2`
- Source file: `n8n-nodes-base/dist/nodes/Perplexity/Perplexity.node.js`
- Node version: `1`
- Groups: `transform`
...
|---|---|---|---|---|---|
| Resource | `resource` | `hidden` | no | `chat` |  |
| Operation | `operation` | `options` | no | `complete` |  |
| Model | `model` | `options` | yes | `sonar` | The model which will generate the completion |
| Messages | `messages` | `fixedCollection` | yes | `{"message":[{"role":"user","content":""}]}` | Any optional system messages must be sent first, followed by alternating user and assistant messages |
| Simplify Output | `simplify` | `boolean` | no | `false` | Whether to return only essential fields (ID, citations, message) |

...
- Source file: `n8n-nodes-base/dist/nodes/WhatsApp/WhatsApp.node.js`
- Node version: `[1,1.1]`
- Groups: `output`
...
### Operations
- resource `media`: `mediaDelete`, `mediaUpload`, `mediaUrlGet`
- resource `message`: `send`, `sendAndWait`, `sendTemplate`

## Parameters
| Display Name | Name | Type | Required | Default | Description |
|---|---|---|---|---|---|
| Resource | `resource` | `options` | no | `message` |  |
| Operation | `operation` | `options` | no | `sendTemplate` |  |
| Messaging Product | `messagingProduct` | `hidden` | no | `whatsapp` |  |
| Sender Phone Number (or ID) | `phoneNumberId` | `options` | yes |  | The ID of the business account's phone number from which the message will be sent from |
| Recipient's Phone Number | `recipientPhoneNumber` | `string` | yes |  | Phone number of the recipient of the message |
| MessageType | `messageType` | `options` | no | `text` | The type of the message |
| Operation | `operation` | `options` | no | `mediaUpload` | The operation to perform on the media |
| Name | `name` | `fixedCollection` | no | `{}` |  |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Longitude | `longitude` | `number` | yes |  |  |
| Latitude | `latitude` | `number` | yes |  |  |
| Additional Fields | `additionalFields` | `fixedCollection` | no | `{}` |  |
| Text Body | `textBody` | `string` | yes |  | The body of the message (max 4096 characters) |
| Take Audio From | `mediaPath` | `options` | no | `useMediaLink` | Use a link, an ID, or n8n to upload an audio file |
| Take Document From | `mediaPath` | `options` | no | `useMediaLink` | Use a link, an ID, or n8n to upload a document |
| Take Image From | `mediaPath` | `options` | no | `useMediaLink` | Use a link, an ID, or n8n to upload an image |
| Take Video From | `mediaPath` | `options` | no | `useMediaLink` | Use a link, an ID, or n8n to upload a video |
| Link | `mediaLink` | `string` | no |  | Link of the media to be sent |
| ID | `mediaId` | `string` | no |  | ID of the media to be sent |
| Input Data Field Name | `mediaPropertyName` | `string` | yes | `data` | The name of the input field containing the binary file data to be uploaded |
| Filename | `mediaFilename` | `string` | yes |  | The name of the file (required when using a file ID) |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Template | `template` | `options` | yes |  | Name of the template |
| Components | `components` | `fixedCollection` | no | `{}` |  |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Sender Phone Number (or ID) | `phoneNumberId` | `options` | yes |  | The ID of the business account's phone number to store the media |
| Property Name | `mediaPropertyName` | `string` | yes | `data` | Name of the binary property which contains the data for the file to be uploaded |
| Media ID | `mediaGetId` | `string` | yes |  | The ID of the media |
| Media ID | `mediaDeleteId` | `string` | yes |  | The ID of the media |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |

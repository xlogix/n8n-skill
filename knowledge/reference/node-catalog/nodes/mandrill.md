...
- Source file: `n8n-nodes-base/dist/nodes/Mandrill/Mandrill.node.js`
- Node version: `1`
- Groups: `output`
...
### Operations
- resource `message`: `sendHtml`, `sendTemplate`
...
| Template Name or ID | `template` | `options` | yes |  | The template you want to send. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. |
...
| Merge Vars | `mergeVarsJson` | `json` | no |  | Global merge variables |
| Merge Vars | `mergeVarsUi` | `fixedCollection` | no | `{}` | Per-recipient merge variables |
| Metadata | `metadataUi` | `fixedCollection` | no | `{}` | Metadata an associative array of user metadata. Mandrill will store this metadata and make it available for retrieval. In addition, you can select up to 10 metadata fields to index and make searchable using the Mandrill search api. |
| Metadata | `metadataJson` | `json` | no |  | Metadata an associative array of user metadata. Mandrill will store this metadata and make it available for retrieval. In addition, you can select up to 10 metadata fields to index and make searchable using the Mandrill search api. |
| Attachments | `attachmentsJson` | `json` | no |  | An array of supported attachments to add to the message |
| Attachments | `attachmentsUi` | `fixedCollection` | no | `{}` | Array of supported attachments to add to the message |
| Headers | `headersJson` | `json` | no |  | Optional extra headers to add to the message (most headers are allowed) |
| Headers | `headersUi` | `fixedCollection` | no | `{}` | Optional extra headers to add to the message (most headers are allowed) |

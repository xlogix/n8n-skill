...
- Node name: `gSuiteAdmin`
- n8n-nodes-base version: `2.7.2`
- Source file: `n8n-nodes-base/dist/nodes/Google/GSuiteAdmin/GSuiteAdmin.node.js`
- Node version: `1`
- Groups: `input`
...
### Operations
- resource `device`: `changeStatus`, `get`, `getAll`, `update`
- resource `group`: `create`, `delete`, `get`, `getAll`, `update`
- resource `user`: `addToGroup`, `create`, `delete`, `get`, `getAll`, `removeFromGroup`, `update`
...
| Device | `deviceId` | `resourceLocator` | yes | `{"mode":"list","value":""}` | Select the device you want to retrieve |
...
| Output | `projection` | `options` | yes | `basic` | What subset of fields to fetch for this device |
| Include Children | `includeChildOrgunits` | `boolean` | no | `false` | Whether to include devices from organizational units below your specified organizational unit |
| Filter | `filter` | `collection` | no | `{}` |  |
| Sort | `sort` | `fixedCollection` | no | `{}` | Define sorting rules for the results |
| Update Fields | `updateOptions` | `collection` | no | `{}` |  |
| Status | `action` | `options` | yes | `reenable` | Set the status of a device |
| Operation | `operation` | `options` | no | `create` |  |
| Group | `groupId` | `resourceLocator` | yes | `{"mode":"list","value":""}` | Select the group to perform the operation on |
| Group Name | `name` | `string` | no |  | The group's display name |
| Group Email | `email` | `string` | yes |  | The group's email address. If your account has multiple domains, select the appropriate domain for the email address. The email must be unique |
...
| Operation | `operation` | `options` | no | `create` |  |
| User | `userId` | `resourceLocator` | yes | `{"mode":"list","value":""}` | Select the user to perform the operation on |
| Group | `groupId` | `resourceLocator` | yes | `{"mode":"list","value":""}` | Select the group to perform the operation on |
| First Name | `firstName` | `string` | yes |  |  |
| Last Name | `lastName` | `string` | yes |  |  |
| Password | `password` | `string` | yes |  | Stores the password for the user account. A minimum of 8 characters is required. The maximum length is 100 characters. |
| Username | `username` | `string` | no |  | The username that will be set to the user. Example: If you domain is example.com and you set the username to n.smith then the user's final email address will be n.smith@example.com. |
| Domain Name or ID | `domain` | `options` | yes |  | Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a> |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Output | `output` | `options` | yes | `simplified` |  |
| Fields | `fields` | `multiOptions` | no | `[]` | Fields to include in the response when "Select Included Fields" is chosen |
| Custom Fields | `projection` | `options` | yes | `basic` | What subset of fields to fetch for this user |
| Custom Schema Names or IDs | `customFieldMask` | `multiOptions` | yes | `[]` | A comma-separated list of schema names. All fields from these schemas are fetched. Choose from the list, or specify IDs using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `100` | Max number of results to return |
| Output | `output` | `options` | yes | `simplified` |  |
| Fields | `fields` | `multiOptions` | no | `[]` | Fields to include in the response when "Select Included Fields" is chosen |

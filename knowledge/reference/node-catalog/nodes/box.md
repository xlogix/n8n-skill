...
- Source file: `n8n-nodes-base/dist/nodes/Box/Box.node.js`
- Node version: `1`
- Groups: `input`
...
### Operations
- resource `file`: `copy`, `delete`, `download`, `get`, `search`, `share`, `upload`
- resource `folder`: `create`, `delete`, `get`, `search`, `share`, `update`
...
| Parent ID | `parentId` | `string` | no |  | The ID of folder to copy the file to. If not defined will be copied to the root folder. |
...
| File ID | `fileId` | `string` | no |  | Field ID |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Query | `query` | `string` | no |  | The string to search for. This query is matched against item names, descriptions, text content of files, and various other fields of the different item types. |
...
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| File ID | `fileId` | `string` | no |  | The ID of the file to share |
| Accessible By | `accessibleBy` | `options` | no |  | The type of object the file will be shared with |
| Use Email | `useEmail` | `boolean` | no | `true` | Whether identify the user by email or ID |
| Email | `email` | `string` | no |  | The user's email address to share the file with |
| User ID | `userId` | `string` | no |  | The user's ID to share the file with |
| Group ID | `groupId` | `string` | no |  | The group's ID to share the file with |
| Role | `role` | `options` | no | `editor` | The level of access granted |
...
| Parent ID | `parentId` | `string` | no |  | ID of the parent folder that will contain the file. If not it will be uploaded to the root folder. |
| Operation | `operation` | `options` | no | `create` |  |
| Name | `name` | `string` | yes |  | Folder's name |
| Parent ID | `parentId` | `string` | no |  | ID of the folder you want to create the new folder in. if not defined it will be created on the root folder. |
| Options | `options` | `collection` | no | `{}` |  |
| Folder ID | `folderId` | `string` | no |  |  |
| Folder ID | `folderId` | `string` | no |  |  |
| Recursive | `recursive` | `boolean` | no | `false` | Whether to delete a folder that is not empty by recursively deleting the folder and all of its content |
| Query | `query` | `string` | no |  | The string to search for. This query is matched against item names, descriptions, text content of files, and various other fields of the different item types. |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `100` | Max number of results to return |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Folder ID | `folderId` | `string` | no |  | The ID of the folder to share |
| Accessible By | `accessibleBy` | `options` | no | `user` | The type of object the file will be shared with |
| Use Email | `useEmail` | `boolean` | no | `true` | Whether identify the user by email or ID |
| Email | `email` | `string` | no |  | The user's email address to share the folder with |
| User ID | `userId` | `string` | no |  | The user's ID to share the folder with |
| Group ID | `groupId` | `string` | no |  | The group's ID to share the folder with |
| Role | `role` | `options` | no | `editor` | The level of access granted |
| Options | `options` | `collection` | no | `{}` |  |
| Folder ID | `folderId` | `string` | yes |  |  |

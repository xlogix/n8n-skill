...
- Source file: `n8n-nodes-base/dist/nodes/Autopilot/Autopilot.node.js`
- Node version: `1`
- Groups: `input`
...
- resource `contactJourney`: `add`
- resource `contactList`: `add`, `exist`, `getAll`, `remove`
- resource `list`: `create`, `getAll`
...
| Email | `email` | `string` | yes |  | Email address of the contact |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Contact ID | `contactId` | `string` | yes |  | Can be ID or email |
| Contact ID | `contactId` | `string` | yes |  | Can be ID or email |
...
| Trigger Name or ID | `triggerId` | `options` | yes |  | List ID. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. |
| Contact ID | `contactId` | `string` | yes |  | Can be ID or email |
| Operation | `operation` | `options` | no | `add` |  |
| List Name or ID | `listId` | `options` | yes |  | ID of the list to operate on. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. |
| Contact ID | `contactId` | `string` | yes |  | Can be ID or email |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `100` | Max number of results to return |
| Operation | `operation` | `options` | no | `create` |  |
| Name | `name` | `string` | yes |  | Name of the list to create |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `100` | Max number of results to return |

...
- Node name: `awsDynamoDb`
- n8n-nodes-base version: `2.7.2`
- Source file: `n8n-nodes-base/dist/nodes/Aws/DynamoDB/AwsDynamoDB.node.js`
- Node version: `1`
- Groups: `transform`
...
### Operations
- resource `item`: `delete`, `get`, `getAll`, `upsert`
...
| Table Name or ID | `tableName` | `options` | yes | `[]` | Table to operate on. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. |
| Data to Send | `dataToSend` | `options` | no | `defineBelow` | Whether to insert the input data this node receives in the new row |
...
| Return | `returnValues` | `options` | no | `NONE` | Use ReturnValues if you want to get the item attributes as they appeared before they were deleted |
| Keys | `keysUi` | `fixedCollection` | no | `{}` | Item's primary key. For example, with a simple primary key, you only need to provide a value for the partition key. For a composite primary key, you must provide values for both the partition key and the sort key. |
| Simplify | `simple` | `boolean` | no | `true` | Whether to return a simplified version of the response instead of the raw data |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Select | `select` | `options` | no | `ALL_ATTRIBUTES` |  |
| Simplify | `simple` | `boolean` | no | `true` | Whether to return a simplified version of the response instead of the raw data |
| Keys | `keysUi` | `fixedCollection` | no | `{}` | Item's primary key. For example, with a simple primary key, you only need to provide a value for the partition key. For a composite primary key, you must provide values for both the partition key and the sort key. |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Scan | `scan` | `boolean` | no | `false` | Whether to do an scan or query. Check <a href="https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/bp-query-scan.html" >differences</a>. |
| Filter Expression | `filterExpression` | `string` | no |  | A filter expression determines which items within the Scan results should be returned to you. All of the other results are discarded. Empty value will return all Scan results. |
| Key Condition Expression | `keyConditionExpression` | `string` | yes |  | Condition to determine the items to be retrieved. The condition must perform an equality test on a single partition key value, in this format: <code>partitionKeyName = :partitionkeyval</code> |
| Expression Attribute Values | `eavUi` | `fixedCollection` | yes | `{}` | Substitution tokens for attribute names in an expression |

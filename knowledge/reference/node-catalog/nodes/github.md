...
- Source file: `n8n-nodes-base/dist/nodes/Github/Github.node.js`
- Node version: `[1,1.1]`
- Groups: `input`
...
- resource `organization`: `getRepositories`
- resource `release`: `create`, `delete`, `get`, `getAll`, `update`
- resource `repository`: `get`, `getIssues`, `getLicense`, `getProfile`, `getPullRequests`, `listPopularPaths`, `listReferrers`
- resource `review`: `create`, `get`, `getAll`, `update`
- resource `user`: `getRepositories`, `getUserIssues`, `invite`
- resource `workflow`: `disable`, `dispatch`, `dispatchAndWait`, `enable`, `get`, `getUsage`, `list`
...
| Operation | `operation` | `options` | no | `create` |  |
| Operation | `operation` | `options` | no | `create` |  |
| Operation | `operation` | `options` | no | `dispatch` |  |
| Your execution will pause until a webhook is called. This URL will be generated at runtime and passed to your Github workflow as a resumeUrl input. | `webhookNotice` | `notice` | no |  |  |
| Repository Owner | `owner` | `resourceLocator` | yes | `{"mode":"list","value":""}` |  |
| Repository Name | `repository` | `resourceLocator` | yes | `{"mode":"list","value":""}` |  |
| Workflow | `workflowId` | `resourceLocator` | yes | `{"mode":"list","value":""}` | The workflow to dispatch |
| Ref | `ref` | `string` | yes | `main` | The git reference for the workflow dispatch (branch or tag name) |
| Ref | `ref` | `resourceLocator` | yes | `{"mode":"list","value":""}` | The git reference for the workflow dispatch (branch, tag, or commit SHA) |
| Inputs | `inputs` | `json` | no | `{}` | JSON object with input parameters for the workflow |
| File Path | `filePath` | `string` | yes |  | The file path of the file. Has to contain the full path. |
...
| Assignees | `assignees` | `collection` | no | `{"assignee":""}` |  |
...
| Issue Number | `issueNumber` | `number` | yes | `0` | The issue number to get data for |
| Issue Number | `issueNumber` | `number` | yes | `0` | The issue number to lock |
| Lock Reason | `lockReason` | `options` | no | `resolved` | The reason for locking the issue |
| Tag | `releaseTag` | `string` | yes |  | The tag of the release |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Release ID | `release_id` | `string` | yes |  |  |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
...
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `50` | Max number of results to return. Maximum value is <a href="https://docs.github.com/en/rest/pulls/pulls?apiVersion=2022-11-28#list-pull-requests">100</a>. |
| Filters | `getRepositoryPullRequestsFilters` | `collection` | no | `{}` |  |
| PR Number | `pullRequestNumber` | `number` | yes | `0` | The number of the pull request |
| Review ID | `reviewId` | `string` | yes |  | ID of the review |
| PR Number | `pullRequestNumber` | `number` | yes | `0` | The number of the pull request |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `50` | Max number of results to return |
| PR Number | `pullRequestNumber` | `number` | yes | `0` | The number of the pull request to review |
| Event | `event` | `options` | no | `approve` | The review action you want to perform |
| Body | `body` | `string` | no |  | The body of the review (required for events Request Changes or Comment) |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Body | `body` | `string` | no |  | The body of the review |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `50` | Max number of results to return |
| Organization | `organization` | `string` | yes |  | The GitHub organization that the user is being invited to |
| Email | `email` | `string` | yes |  | The email address of the invited user |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `50` | Max number of results to return |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `50` | Max number of results to return |
| Filters | `getUserIssuesFilters` | `collection` | no | `{}` |  |

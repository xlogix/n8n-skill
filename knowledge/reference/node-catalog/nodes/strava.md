...
- Source file: `n8n-nodes-base/dist/nodes/Strava/Strava.node.js`
- Node version: `[1,1.1]`
- Groups: `input`
...
### Operations
- resource `activity`: `create`, `get`, `getAll`, `getComments`, `getKudos`, `getLaps`, `getStreams`, `getZones`, `update`
...
| Name | `name` | `string` | yes |  | The name of the activity |
| Type | `type` | `string` | yes |  | Type of activity. For example - Run, Ride etc. |
| Sport Type | `sport_type` | `options` | no | `Run` | Type of sport |
| Start Date | `startDate` | `dateTime` | yes |  | ISO 8601 formatted date time |
| Elapsed Time (Seconds) | `elapsedTime` | `number` | yes | `0` | In seconds |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Activity ID | `activityId` | `string` | yes |  | ID or email of activity |
...
| Keys | `keys` | `multiOptions` | yes | `[]` | Desired stream types to return |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `50` | Max number of results to return |

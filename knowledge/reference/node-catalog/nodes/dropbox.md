...
- Source file: `n8n-nodes-base/dist/nodes/Dropbox/Dropbox.node.js`
- Node version: `1`
- Groups: `input`
...
### Operations
- resource `file`: `copy`, `delete`, `download`, `move`, `upload`
- resource `folder`: `copy`, `create`, `delete`, `list`, `move`
- resource `search`: `query`

## Parameters
| Display Name | Name | Type | Required | Default | Description |
|---|---|---|---|---|---|
| Authentication | `authentication` | `options` | no | `accessToken` | Means of authenticating with the service |
...
| From Path | `path` | `string` | yes |  | The path of file or folder to copy |
| To Path | `toPath` | `string` | yes |  | The destination path of file or folder |
| Delete Path | `path` | `string` | yes |  | The path to delete. Can be a single file or a whole folder. |
| From Path | `path` | `string` | yes |  | The path of file or folder to move |
| To Path | `toPath` | `string` | yes |  | The new path of file or folder |
| File Path | `path` | `string` | yes |  | The file path of the file to download. Has to contain the full path. |
| Put Output File in Field | `binaryPropertyName` | `string` | yes | `data` |  |
| File Path | `path` | `string` | yes |  | The file path of the file to upload. Has to contain the full path. The parent folder has to exist. Existing files get overwritten. |
...
| Query | `query` | `string` | yes |  | The string to search for. May match across multiple fields based on the request arguments. |
| File Status | `fileStatus` | `options` | no | `active` | The string to search for. May match across multiple fields based on the request arguments. |
...
| Folder | `path` | `string` | yes |  | The folder to create. The parent folder has to exist. |
| Folder Path | `path` | `string` | no |  | The path of which to list the content |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `100` | Max number of results to return |
| Filters | `filters` | `collection` | no | `{}` |  |

...
- Node name: `httpRequest`
- n8n-nodes-base version: `2.7.2`
- Source file: `n8n-nodes-base/dist/nodes/HttpRequest/HttpRequest.node.js`
- Node version: `[3,4,4.1,4.2,4.3,4.4]`
- Groups: `output`
...
|---|---|---|---|---|---|
|  | `curlImport` | `curlImport` | no |  |  |
| Method | `method` | `options` | no | `GET` | The request method to use |
| URL | `url` | `string` | yes |  | The URL to make the request to |
| Authentication | `authentication` | `options` | no | `none` |  |
| Credential Type | `nodeCredentialType` | `credentialsSelect` | yes |  |  |
| Make sure you have specified the scope(s) for the Service Account in the credential | `googleApiWarning` | `notice` | no |  |  |
| Generic Auth Type | `genericAuthType` | `credentialsSelect` | yes |  |  |
| SSL Certificates | `provideSslCertificates` | `boolean` | no | `false` |  |
| Provide certificates in node's 'Credential for SSL Certificates' parameter | `provideSslCertificatesNotice` | `notice` | no |  |  |
| SSL Certificate | `sslCertificate` | `credentials` | no |  |  |
| Send Query Parameters | `sendQuery` | `boolean` | no | `false` | Whether the request has query params or not |
| Specify Query Parameters | `specifyQuery` | `options` | no | `keypair` |  |
| Query Parameters | `queryParameters` | `fixedCollection` | no | `{"parameters":[{"name":"","value":""}]}` |  |
| JSON | `jsonQuery` | `json` | no |  |  |
| Send Headers | `sendHeaders` | `boolean` | no | `false` | Whether the request has headers or not |
| Specify Headers | `specifyHeaders` | `options` | no | `keypair` |  |
| Headers | `headerParameters` | `fixedCollection` | no | `{"parameters":[{"name":"","value":""}]}` |  |
| JSON | `jsonHeaders` | `json` | no |  |  |
| Send Body | `sendBody` | `boolean` | no | `false` | Whether the request has a body or not |
| Body Content Type | `contentType` | `options` | no | `json` | Content-Type to use to send body parameters |
| Specify Body | `specifyBody` | `options` | no | `keypair` | The body can be specified using explicit fields (<code>keypair</code>) or using a JavaScript object (<code>json</code>) |
| Body Parameters | `bodyParameters` | `fixedCollection` | no | `{"parameters":[{"name":"","value":""}]}` |  |
| JSON | `jsonBody` | `json` | no |  |  |
| Body | `bodyParameters` | `fixedCollection` | no | `{"parameters":[{"name":"","value":""}]}` |  |
| Specify Body | `specifyBody` | `options` | no | `keypair` |  |
| Body Fields | `bodyParameters` | `fixedCollection` | no | `{"parameters":[{"name":"","value":""}]}` |  |
| Body | `body` | `string` | no |  |  |
| Input Data Field Name | `inputDataFieldName` | `string` | no |  | The name of the incoming field containing the binary file data to be processed |
| Content Type | `rawContentType` | `string` | no |  |  |
| Body | `body` | `string` | no |  |  |
| Options | `options` | `collection` | no | `{}` |  |
| Optimize Response | `optimizeResponse` | `boolean` | no | `false` | Whether the optimize the tool response to reduce amount of data passed to the LLM that could lead to better result and reduce cost |
| Expected Response Type | `responseType` | `options` | no | `json` |  |
| Field Containing Data | `dataField` | `string` | no |  | Specify the name of the field in the response containing the data |
| Include Fields | `fieldsToInclude` | `options` | no | `all` | What fields response object should include |
| Fields | `fields` | `string` | no |  | Comma-separated list of the field names. Supports dot notation. You can drag the selected fields from the input panel. |
| Selector (CSS) | `cssSelector` | `string` | no | `body` | Select specific element(e.g. body) or multiple elements(e.g. div) of chosen type in the response HTML. |
| Return Only Content | `onlyContent` | `boolean` | no | `false` | Whether to return only content of html elements, stripping html tags and attributes |
| Elements To Omit | `elementsToOmit` | `string` | no |  | Comma-separated list of selectors that would be excluded when extracting content |
| Truncate Response | `truncateResponse` | `boolean` | no | `false` |  |
| Max Response Characters | `maxLength` | `number` | no | `1000` |  |
| You can view the raw requests this node makes in your browser's developer console | `infoMessage` | `notice` | no |  |  |

...
- Source file: `n8n-nodes-base/dist/nodes/Supabase/Supabase.node.js`
- Node version: `1`
- Groups: `input`
...
|---|---|---|---|---|---|
| Use Custom Schema | `useCustomSchema` | `boolean` | no | `false` | Whether to use a database schema different from the default "public" schema (requires schema exposure in the <a href="https://supabase.com/docs/guides/api/using-custom-schemas?queryGroups=language&language=curl#exposing-custom-schemas">Supabase API</a>) |
| Schema | `schema` | `string` | no | `public` | Name of database schema to use for table |
| Resource | `resource` | `options` | no | `row` |  |
| Operation | `operation` | `options` | no | `create` |  |
| Table Name or ID | `tableId` | `options` | yes |  | Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a> |
| Select Type | `filterType` | `options` | no | `manual` |  |
| Must Match | `matchType` | `options` | no | `anyFilter` |  |
| Select Conditions | `filters` | `fixedCollection` | no | `{}` | Filter to decide which rows get updated |
| See <a href="https://postgrest.org/en/stable/references/api/tables_views.html#horizontal-filtering" target="_blank">PostgREST guide</a> to creating filters | `jsonNotice` | `notice` | no |  |  |
| Filters (String) | `filterString` | `string` | no |  |  |
| Data to Send | `dataToSend` | `options` | no | `defineBelow` |  |
...
| Select Type | `filterType` | `options` | no | `manual` |  |
| Must Match | `matchType` | `options` | no | `anyFilter` |  |
| Select Conditions | `filters` | `fixedCollection` | no | `{}` | Filter to decide which rows get deleted |
| See <a href="https://postgrest.org/en/stable/references/api/tables_views.html#horizontal-filtering" target="_blank">PostgREST guide</a> to creating filters | `jsonNotice` | `notice` | no |  |  |
| Filters (String) | `filterString` | `string` | no |  |  |
| Select Conditions | `filters` | `fixedCollection` | no | `{}` |  |
...
| Filter | `filterType` | `options` | no | `manual` |  |
| Must Match | `matchType` | `options` | no | `anyFilter` |  |
| Filters | `filters` | `fixedCollection` | no | `{}` | Filter to decide which rows get retrieved |
| See <a href="https://postgrest.org/en/stable/references/api/tables_views.html#horizontal-filtering" target="_blank">PostgREST guide</a> to creating filters | `jsonNotice` | `notice` | no |  |  |
| Filters (String) | `filterString` | `string` | no |  |  |

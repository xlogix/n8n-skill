...
- Source file: `n8n-nodes-base/dist/nodes/Paddle/Paddle.node.js`
- Node version: `1`
- Groups: `output`
...
### Operations
- resource `coupon`: `create`, `getAll`, `update`
- resource `payment`: `getAll`, `reschedule`
- resource `plan`: `get`, `getAll`
...
|---|---|---|---|---|---|
| Resource | `resource` | `options` | no | `coupon` |  |
| Operation | `operation` | `options` | no | `create` |  |
| Coupon Type | `couponType` | `options` | no | `checkout` | Either product (valid for specified products or subscription plans) or checkout (valid for any checkout) |
| Product Names or IDs | `productIds` | `multiOptions` | yes | `[]` | Comma-separated list of product IDs. Required if coupon_type is product. Choose from the list, or specify IDs using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. |
| Discount Type | `discountType` | `options` | no | `flat` | Either flat or percentage |
| Discount Amount Currency | `discountAmount` | `number` | no | `1` | Discount amount in currency |
| Discount Amount % | `discountAmount` | `number` | no | `1` | Discount amount in percentage |
| Currency | `currency` | `options` | no | `EUR` | The currency must match the balance currency specified in your account |
| JSON Parameters | `jsonParameters` | `boolean` | no | `false` |  |
| Additional Fields | `additionalFieldsJson` | `json` | no |  | Attributes in JSON form |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Product ID | `productId` | `string` | yes |  | The specific product/subscription ID |
...
| Update By | `updateBy` | `options` | no | `couponCode` | Either flat or percentage |
| Coupon Code | `couponCode` | `string` | no |  | Identify the coupon to update |
| Group | `group` | `string` | no |  | The name of the group of coupons you want to update |
| JSON Parameters | `jsonParameters` | `boolean` | no | `false` |  |
| Additional Fields | `additionalFieldsJson` | `json` | no |  | Attributes in JSON form |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
...
| Additional Fields | `additionalFieldsJson` | `json` | no |  | Attributes in JSON form |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |
| Payment Name or ID | `paymentId` | `options` | yes |  | The upcoming subscription payment ID. Choose from the list, or specify an ID using an <a href="https://docs.n8n.io/code/expressions/">expression</a>. |
| Date | `date` | `dateTime` | no |  | Date you want to move the payment to |
| Operation | `operation` | `options` | no | `get` |  |
| Plan ID | `planId` | `string` | yes |  | Filter: The subscription plan ID |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | no | `100` | Max number of results to return |
| Operation | `operation` | `options` | no | `getAll` |  |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
...
| Operation | `operation` | `options` | no | `getAll` |  |
| Return All | `returnAll` | `boolean` | no | `false` | Whether to return all results or only up to a given limit |
| Limit | `limit` | `number` | yes | `100` | Max number of results to return |
| JSON Parameters | `jsonParameters` | `boolean` | no | `false` |  |
| Additional Fields | `additionalFieldsJson` | `json` | no |  | Attributes in JSON form |
| Additional Fields | `additionalFields` | `collection` | no | `{}` |  |

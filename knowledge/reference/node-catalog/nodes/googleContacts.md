...
- Node name: `googleContacts`
- n8n-nodes-base version: `2.7.2`
- Source file: `n8n-nodes-base/dist/nodes/Google/Contacts/GoogleContacts.node.js`
- Node version: `1`
- Groups: `input`
...
## Credentials
- `googleContactsOAuth2Api` (required)
...
| Contact ID | `contactId` | `string` | yes |  |  |
| Fields | `fields` | `multiOptions` | no | `[]` | A field mask to restrict which fields on each person are returned. Multiple fields can be specified by separating them with commas. |
...
| Fields | `fields` | `multiOptions` | no | `[]` | A field mask to restrict which fields on each person are returned. Multiple fields can be specified by separating them with commas. |
| Use Query | `useQuery` | `boolean` | no | `false` | Whether or not to use a query to filter the results |
| Query | `query` | `string` | no |  | The plain-text query for the request. The query is used to match prefix phrases of the fields on a person. For example, a person with name "foo name" matches queries such as "f", "fo", "foo", "foo n", "nam", etc., but not "oo n". |
| RAW Data | `rawData` | `boolean` | no | `false` | Whether to return the data exactly in the way it got received from the API |
